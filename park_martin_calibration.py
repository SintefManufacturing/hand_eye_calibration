...
class ParkMartinCalibrator(object):
    ...
    class Exception(Exception):
        ...
        def __repr__(self):
            return ('ParkMartinCalibratorException : {}'
                    .format(self._msg))
    ...
    def __iadd__(self, pose_pairs):
        ...
        if pp_shape == (2,):
            ...
            raise ParkMartinCalibrator.Exception(
                'The given pose pair(s) was not well formated; needs (N,2) or'
                + ' (2,) np.array of Transforms.')
    ...
    def M(self):
        ...
        if self._m is None:
            self._m = np.sum([np.outer(sm.rot, fm.rot)
                              for fm, sm in self._move_pairs], axis=0)
        return self._m
    ...
    def MTM_SVD(self):
        ...
        if self._mtm_svd is None:
            self._mtm_svd = np.linalg.svd(self.M.T.dot(self.M),
                                          full_matrices=False)
    ...
    def C(self):
        ...
        # C is the vertical stacking of I - Theta_A_i, the
        # orientations of the flange moves
        if self._c is None:
            I_3 = np.identity(3)
            self._c = np.vstack([I_3 - fm.orient._data
    ...
    def pos_sif(self):
        if self._pos_sif is None:
            ...
            d = np.hstack([(fm.d - self.orient_sif*sm.d)._data
                           for fm, sm in self._move_pairs])
            # The solution to the position, b_X, is now computed as
            # (C^T*C)^(-1)*C*d
        ...
        if self._sensor_in_flange is None:
            if self._pose_pairs is None:
                raise Exception('The sensor in flange transform has not been'
                                + ' computed yet, and flange-sensor pose pairs'
                                + ' have not been set!!!')
            else:
                self._sensor_in_flange = m3d.Transform(self.orient_sif,
                                                       self.pos_sif)
